...
private:
  ...
  const uint16_t sep_; //报文的分隔符，0-无分隔符；1-四字节的包头;2-"\r\n\r\n"分隔符（http协议）
  
public:
  Buffer(uint16_t sep=1);
  ~Buffer();
  
  void append(const char* data, size_t size);  //把数据添加到buf_中
  void appendwithsep(const char *data,size_t size); //把数据追加到buf_中
  void erase(size_t pos, size_t nn);         //从buf_的pos开始，删除nn个字节，pos从0开始
  size_t size();          //返回buf的大小
